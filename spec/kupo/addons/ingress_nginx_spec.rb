require "kupo/addon"
require "kupo/addons/ingress_nginx"

describe Kupo::Addons::IngressNginx do
  ...
  describe "#image_name" do
    ...
    it "returns default for arm64" do
      ...
      expect(subject.image_name).to eq(Kupo::Addons::IngressNginx::DEFAULT_BACKEND_ARM64_IMAGE)
    end

    it "returns configured name" do
      expect(cpu_arch).to receive(:name).and_return('amd64')
      subject = described_class.new(host, {})
      expect(subject.image_name).to eq(Kupo::Addons::IngressNginx::DEFAULT_BACKEND_IMAGE)
    end
  end
end