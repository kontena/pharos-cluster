...
describe Kupo::YamlFile do
  ...
  context 'for an erb file with .erb extension' do
    ...
    it 'has the erb whitespace trimming enabled' do
      expect(subject.erb_result).to eq File.read(fixtures_dir('yaml/no_erb.yml'))
    end
  ...
  context 'with file input' do
    let(:file) { File.open(fixtures_dir('yaml/with_erb.yml.erb')) }
    ...
    it 'reads the file' do
      ...
      subject.load
      expect(subject.filename).to eq fixtures_dir('yaml/with_erb.yml.erb')
    end
  end

  context 'with content input' do
    let(:filename) { fixtures_dir('yaml/no_erb.yml') }
