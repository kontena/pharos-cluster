...
module Pharos
  class ClusterManager
    ...
    def apply_phases
      ...
      apply_phase(Phases::ConfigureDNS, [master_hosts.first], kube: true)
      apply_phase(Phases::ConfigureWeave, [master_hosts.first], kube: true) if config.network.provider == 'weave'
      apply_phase(Phases::ConfigureCalico, [master_hosts.first], kube: true) if config.network.provider == 'calico'
      apply_phase(Phases::ConfigureMetrics, [master_hosts.first], kube: true)
