  ...
  module Addons
    class KubeDashboard < Pharos::Addon
      ...
      def install
        ...
        Out.info { "~~> kubernetes-dashboard can be accessed via kubectl proxy at http://localhost:8001/ui" }
        ...
        Out.info { "~~> kubernetes-dashboard admin token can be fetched using: kubectl describe secret #{token_secret.name} -n kube-system" }
      end
    end
  end
end
