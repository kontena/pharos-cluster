  ...
  module Host
    class El7 < Configurer
      ...
      def configure_container_runtime_safe?
        ...
        if docker?
          return true if host.exec("rpm -qi docker").error? # docker not installed
          return true if host.exec("rpm -qi docker-#{DOCKER_VERSION}").success?
        elsif crio?
          bin_exist = host.file('/usr/local/bin/crio').exist?
          return true if host.exec("rpm -qi cri-o").error? && !bin_exist # cri-o not installed
          return true if host.exec("rpm -qi cri-o-#{Pharos::CRIO_VERSION}").success?
        end
