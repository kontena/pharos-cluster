...
if [ ! -f /usr/local/bin/cfssl ]; then
    curl -s -L -o /usr/local/bin/cfssl "https://dl.bintray.com/pharos-bin/cfssl/1.2/cfssl_linux-${ARCH}"
...
if [ ! -f /usr/local/bin/cfssljson ]; then
    curl -s -L -o /usr/local/bin/cfssljson "https://dl.bintray.com/pharos-bin/cfssl/cfssljson_linux-${ARCH}"
