    ...
    class GatherFacts < Pharos::Phase
      ...
      def hostname
        ...
        if FULL_HOSTNAME_CLOUD_PROVIDERS.include?(cloud_provider)
          host.exec!('hostname -f').strip
        else
          host.exec!('hostname -s').strip
        end.downcase
      end
      ...
      def host_checks
        data = {}
        data['kubelet_configured'] = host.file('/etc/kubernetes/kubelet.conf').exist?
        data['ca_exists'] = host.file('/etc/kubernetes/pki/ca.key').exist?
        data['etcd_ca_exists'] = host.file('/etc/pharos/pki/ca-key.pem').exist?

        if data['ca_exists']
          result = host.exec("sudo curl -sSf --connect-timeout 1 --cacert /etc/kubernetes/pki/ca.crt https://localhost:6443/healthz")
      ...
      def private_interface_address(interface)
        host.exec!("ip -o addr show dev #{interface} scope global").each_line do |line|
      ...
      def resolvconf_nameservers
        @resolvconf_nameservers ||= host.file('/etc/resolv.conf').lines.map { |l| l[/^nameserver ([\h:.]+)/, 1] }.compact
      end
      ...
      def resolvconf_systemd_resolved_stub?
        !!host.file('/etc/resolv.conf').readlink && resolvconf_nameservers.include?('127.0.0.53')
      end
      ...
      def read_routes
        routes = []
        host.exec!("sudo ip route").each_line do |line|
