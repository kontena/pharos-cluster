...
module Pharos
  module Phases
    class UpgradeMaster < Pharos::Phase
      ...
      def upgrade?
        file = host.file('/etc/kubernetes/manifests/kube-apiserver.yaml')
      ...
      def pod_security_disabled?
        api_manifest = host.file('/etc/kubernetes/manifests/kube-apiserver.yaml')
      ...
      def upgrade
        ...
        host.tempfile(content: cfg, prefix: "kubeadm.cfg") do |tmp_file|
          host.exec!("sudo /usr/local/bin/pharos-kubeadm-#{Pharos::KUBEADM_VERSION} upgrade apply #{Pharos::KUBE_VERSION} -y --ignore-preflight-errors=all --allow-experimental-upgrades --config #{tmp_file}")
        end
