...
module Pharos
  module Phases
    class ConfigureSecretsEncryption < Pharos::Phase
      ...
      def call
        keys = cluster_context.secrets_encryption_keys || read_config_keys || generate_keys

        ensure_config(keys)

        cluster_context.secrets_encryption_keys = keys
      end

      # @return [Hash, nil]
      def read_config_keys
        logger.debug { "Checking if secrets encryption is already configured ..." }
        file = transport.file(SECRETS_CFG_FILE)
      ...
      def ensure_config(keys)
        cfg_file = transport.file(SECRETS_CFG_FILE)
        return if cfg_file.exist?

        logger.info { "Creating secrets encryption configuration ..." }
        transport.exec!("test -d #{SECRETS_CFG_DIR} || sudo install -m 0700 -d #{SECRETS_CFG_DIR}")
